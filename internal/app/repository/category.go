...
func (r *SupportRepo) ReplaceCategoriesTree(ctx context.Context, mainPrompt string, messagesJSON []byte, roots []*datastruct.CategoryNode) error {
	...
	insertNode = func(parentID *int, node *datastruct.CategoryNode) error {
		var id int
		err := tx.QueryRow(ctx, `
			INSERT INTO categories (parent_id, name, prompt_text, manager_id, work_hours) 
			VALUES ($1, $2, $3, $4, $5) RETURNING id`,
			parentID, node.Name, node.PromptText, node.ManagerID, node.WorkHours).Scan(&id)
		if err != nil {
			return err
		}
...
func (r *SupportRepo) GetAllCategoriesFull(ctx context.Context) ([]datastruct.Category, error) {
	rows, err := r.db.Query(ctx, "SELECT id, parent_id, name, prompt_text, manager_id, work_hours FROM categories ORDER BY id")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &c.ParentID, &c.Name, &c.PromptText, &c.ManagerID, &c.WorkHours); err != nil {
			return nil, err
		}
...
func (r *SupportRepo) GetCategoriesByParent(ctx context.Context, parentID *int) ([]datastruct.Category, error) {
	query := "SELECT id, parent_id, name, prompt_text, manager_id, work_hours FROM categories WHERE parent_id IS NULL ORDER BY id"
	args := []any{}

	if parentID != nil {
		query = "SELECT id, parent_id, name, prompt_text, manager_id, work_hours FROM categories WHERE parent_id = $1 ORDER BY id"
...
func (r *SupportRepo) GetCategoryByID(ctx context.Context, id int) (*datastruct.Category, error) {
	var c datastruct.Category
	err := r.db.QueryRow(ctx, "SELECT id, parent_id, name, prompt_text, manager_id, work_hours FROM categories WHERE id = $1", id).
		Scan(&c.ID, &c.ParentID, &c.Name, &c.PromptText, &c.ManagerID, &c.WorkHours)
	return &c, err
}
